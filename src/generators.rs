...
pub trait Generator {
    ...
    fn generate(&self, source: &mut Iterator<Item = u8>) -> Maybe<Self::Item>;
    ...
    fn generate_from(&self, source: &InfoPool) -> Maybe<Self::Item> {
        self.generate(&mut source.replay() as &mut Iterator<Item = u8>)
    }
...
impl<V: Clone> Generator for Const<V> {
    type Item = V;
    fn generate(&self, _: &mut Iterator<Item = u8>) -> Maybe<Self::Item> {
        Ok(self.0.clone())
    }
}
